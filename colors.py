...
def create_csv(data, url):
    global j
    w_csv = data
    header = ["Name", "SKU", "Price", "Quantity", "Description", "Suppliment Facts", "Direction Of Use", "Category",
              "URL"]
    filename = url + ".csv"
    csv_file = open(filename.strip(), "a")
    wr = csv.writer(csv_file, quoting=csv.QUOTE_ALL, delimiter=',', lineterminator='\n')
    if j is 0:
        wr.writerow(header)
        j = j + 1
    wr.writerow(w_csv)
...
if __name__ == '__main__':
    for x in names:
        rgb = rgbcode(x[0])
        create_csv(["#" + x[0], x[1], rgb[0], rgb[1], rgb[2]], "hextorgb")
